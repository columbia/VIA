...
Require Import PSCI.Code.psci_cpu_off.
...
Section CodeProof.
  ...
  Section BodyProof.
    ...
    Lemma psci_cpu_off_body_correct:
      forall m d d' env le rec_base rec_offset
             (Henv: env = PTree.empty _)
             (Hinv: high_level_invariant d)
             (HPTrec: PTree.get _rec le = Some (Vptr rec_base (Int.repr rec_offset)))
             (Hspec: psci_cpu_off_spec0 (rec_base, rec_offset) d = Some d'),
           exists le', (exec_stmt ge env le ((m, d): mem) psci_cpu_off_body E0 le' (m, d') Out_normal).
    Proof.
      solve_code_proof Hspec psci_cpu_off_body; eexists; solve_proof_low.
    Qed.
