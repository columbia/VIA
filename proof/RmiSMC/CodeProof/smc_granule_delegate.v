...
Require Import RmiSMC.Code.smc_granule_delegate.

Require Import RmiSMC.LowSpecs.smc_granule_delegate.
...
Section CodeProof.
  ...
  Section BodyProof.
    ...
    Variable b_granule_delegate_ops: block.
    Hypothesis h_granule_delegate_ops_s : Genv.find_symbol ge _granule_delegate_ops = Some b_granule_delegate_ops.
    Hypothesis h_granule_delegate_ops_p : Genv.find_funct_ptr ge b_granule_delegate_ops
                                          = Some (External (EF_external _granule_delegate_ops
                                                           (signature_of_type (Tcons Tptr (Tcons tulong Tnil)) tvoid cc_default))
                                                 (Tcons Tptr (Tcons tulong Tnil)) tvoid cc_default).
    Local Opaque granule_delegate_ops_spec.

    Lemma smc_granule_delegate_body_correct:
      forall m d d' env le addr res
             (Henv: env = PTree.empty _)
             (Hinv: high_level_invariant d)
             (HPTaddr: PTree.get _addr le = Some (Vlong addr))
             (Hspec: smc_granule_delegate_spec0 (VZ64 (Int64.unsigned addr)) d = Some (d', VZ64 (Int64.unsigned res))),
           exists le', (exec_stmt ge env le ((m, d): mem) smc_granule_delegate_body E0 le' (m, d') (Out_return (Some (Vlong res, tulong)))).
    Proof.
      solve_code_proof Hspec smc_granule_delegate_body; eexists; solve_proof_low.
    Qed.
