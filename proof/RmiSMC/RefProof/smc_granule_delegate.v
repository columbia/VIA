...
Require Import RmiSMC.Specs.smc_granule_delegate.
...
Section Refine.
  ...
  Hint Unfold
       ...
       is_null_spec
       granule_delegate_ops_spec
    .

  Lemma smc_granule_delegate_spec_exists:
    forall habd habd'  labd addr res
           (Hspec: smc_granule_delegate_spec addr habd = Some (habd', res))
            (Hrel: relate_RData habd labd),
    exists labd', smc_granule_delegate_spec0 addr labd = Some (labd', res) /\ relate_RData habd' labd'.
  Proof.
    Local Opaque ptr_eq.
    intros. destruct Hrel. inv id_rdata. duplicate valid_o. destruct D.
    unfold smc_granule_delegate_spec, smc_granule_delegate_spec0 in *.
