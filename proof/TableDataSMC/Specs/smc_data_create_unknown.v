...
Section Spec.
  ...
  Definition smc_data_create_unknown_spec (data_addr: Z64) (rd_addr: Z64) (map_addr: Z64) (adt: RData) : option (RData * Z64) :=
    match data_addr, rd_addr, map_addr with
    | VZ64 data_addr, VZ64 rd_addr, VZ64 map_addr =>
      rely is_int64 data_addr; rely is_int64 rd_addr; rely is_int64 map_addr;
      let data_gidx := __addr_to_gidx data_addr in
      let rd_gidx := __addr_to_gidx rd_addr in
      rely GRANULE_ALIGNED data_addr; rely GRANULE_ALIGNED rd_addr;
      rely is_gidx data_gidx; rely is_gidx rd_gidx;
      ...
      if PTE_TO_IPA_STATE llt_pte =? IPA_STATE_VACANT then
        ...
        Some (adt {log: EVT CPU_ID (REL data_gidx (gdata' {glock: Some CPU_ID})) ::
                            ...
                            EVT CPU_ID (REL llt_gidx gllt' {glock: Some CPU_ID}) :: log adt}
