...
Section Spec.
  ...
  Definition restore_realm_state_spec (rec: Pointer) (adt: RData) : option RData :=
    ...
    rely is_int64 (g_pc (grec gn)); rely is_int64 (g_pstate (grec gn));
    rely is_int64 (r_icc_sre_el2 (cpu_regs (priv adt)));
    rely regs_is_int64_dec sysregs;
    Some adt {priv: (priv adt) {cpu_regs: (cpu_regs (priv adt))
                                            ...
                                            {r_elr_el2: (g_pc (grec gn))}
                                            {r_spsr_el2: (g_pstate (grec gn))}
                                            {r_icc_sre_el2: (Z.land (r_icc_sre_el2 (cpu_regs (priv adt)))  NOT_ICC_SRE_EL2_ENABLE)}}}.
