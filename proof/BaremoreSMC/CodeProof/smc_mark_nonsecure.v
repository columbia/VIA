...
Require Import BaremoreSMC.Code.smc_mark_nonsecure.

Require Import BaremoreSMC.LowSpecs.smc_mark_nonsecure.
...
Section CodeProof.
  ...
  Section BodyProof.
    ...
    Lemma smc_mark_nonsecure_body_correct:
      forall m d d' env le addr
             (Henv: env = PTree.empty _)
             (Hinv: high_level_invariant d)
             (HPTaddr: PTree.get _addr le = Some (Vlong addr))
             (Hspec: smc_mark_nonsecure_spec0 (VZ64 (Int64.unsigned addr)) d = Some d'),
           exists le', (exec_stmt ge env le ((m, d): mem) smc_mark_nonsecure_body E0 le' (m, d') Out_normal).
    Proof.
      solve_code_proof Hspec smc_mark_nonsecure_body; eexists; solve_proof_low.
    Qed.
