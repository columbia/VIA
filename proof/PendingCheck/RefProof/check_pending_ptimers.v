...
Require Import PendingCheck.Specs.check_pending_ptimers.
...
Section Refine.
  ...
  Lemma check_pending_ptimers_spec_exists:
    forall habd habd'  labd rec
           (Hspec: check_pending_ptimers_spec rec habd = Some habd')
            (Hrel: relate_RData habd labd),
    exists labd', check_pending_ptimers_spec0 rec labd = Some labd' /\ relate_RData habd' labd'.
  Proof.
    Local Opaque ptr_eq.
    intros. destruct Hrel. inv id_rdata. destruct rec.
    unfold check_pending_ptimers_spec, check_pending_ptimers_spec0 in *.
    ...
    assert(0 <= 30 / 64 < 512) by (replace (30 / 64) with 0 by reflexivity; omega).
    ...
    repeat swap_fields; repeat simpl_field. rewrite <- C14, <- Prop0; repeat simpl_field.
    reflexivity.
  Qed.
