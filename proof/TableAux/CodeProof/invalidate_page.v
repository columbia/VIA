...
Require Import TableAux.Code.invalidate_page.
...
Section CodeProof.
  ...
  Section BodyProof.
    ...
    Lemma invalidate_page_body_correct:
      forall m d d' env le addr
             (Henv: env = PTree.empty _)
             (Hinv: high_level_invariant d)
             (HPTaddr: PTree.get _addr le = Some (Vlong addr))
             (Hspec: invalidate_page_spec0 (VZ64 (Int64.unsigned addr)) d = Some d'),
           exists le', (exec_stmt ge env le ((m, d): mem) invalidate_page_body E0 le' (m, d') Out_normal).
    Proof.
      solve_code_proof Hspec invalidate_page_body; eexists; solve_proof_low.
    Qed.
