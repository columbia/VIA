...
Require Import BaremoreService.Code.asc_mark_realm.

Require Import BaremoreService.LowSpecs.asc_mark_realm.
...
Section CodeProof.
  ...
  Section BodyProof.
    ...
    Lemma asc_mark_realm_body_correct:
      forall m d d' env le addr res
             (Henv: env = PTree.empty _)
             (Hinv: high_level_invariant d)
             (HPTaddr: PTree.get _addr le = Some (Vlong addr))
             (Hspec: asc_mark_realm_spec0 (VZ64 (Int64.unsigned addr)) d = Some (d', VZ64 (Int64.unsigned res))),
           exists le', (exec_stmt ge env le ((m, d): mem) asc_mark_realm_body E0 le' (m, d') (Out_return (Some (Vlong res, tulong)))).
    Proof.
      solve_code_proof Hspec asc_mark_realm_body; eexists; solve_proof_low.
    Qed.
