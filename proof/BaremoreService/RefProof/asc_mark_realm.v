...
Require Import BaremoreService.Specs.asc_mark_realm.
...
Section Refine.
  ...
  Lemma asc_mark_realm_spec_exists:
    forall habd habd'  labd addr res
           (Hspec: asc_mark_realm_spec addr habd = Some (habd', res))
            (Hrel: relate_RData habd labd),
    exists labd', asc_mark_realm_spec0 addr labd = Some (labd', res) /\ relate_RData habd' labd'.
  Proof.
    intros. destruct Hrel. inv id_rdata.
    unfold asc_mark_realm_spec0, asc_mark_realm_spec in *.
