...
Require Import TableDataOpsRef1.Specs.table_destroy1.
...
Section Refine.
  ...
  Lemma table_destroy1_spec_exists:
    forall habd habd'  labd g_rd map_addr rtt_addr level res
      (Hspec: table_destroy1_spec g_rd map_addr rtt_addr level habd = Some (habd', res))
      (Hrel: relate_RData habd labd),
    exists labd', table_destroy1_spec0 g_rd map_addr rtt_addr level labd = Some (labd', res) /\ relate_RData habd' labd'.
  Proof.
    Local Opaque peq ptr_eq granule_fill_table.fill_table.
    intros. duplicate Hrel. destruct D. clear hrepl lrepl. destruct g_rd.
    unfold table_destroy1_spec, table_destroy1_spec0 in *.
