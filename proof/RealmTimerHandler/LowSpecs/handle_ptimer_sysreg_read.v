...
Section SpecLow.
  ...
  Definition handle_ptimer_sysreg_read_spec0 (rec: Pointer) (esr: Z64) (adt: RData) : option RData :=
    match rec, esr with
    | (_rec_base, _rec_ofst), VZ64 _esr =>
      ...
      if (_ec =? 3209220) then
        when' _t'2 == sysreg_read_spec 34 adt;
        rely is_int64 _t'2;
        when adt == set_rec_regs_spec (_rec_base, _rec_ofst) _rt (VZ64 _t'2) adt;
        Some adt
      else
        if (_ec =? 3340292) then
          when' _cntv_ctl == sysreg_read_spec 32 adt;
          rely is_int64 _cntv_ctl;
          when'' _t'4_base, _t'4_ofst == get_rec_ptimer_spec (_rec_base, _rec_ofst) adt;
          ...
          if (_ec =? 3471364) then
            when' _t'6 == sysreg_read_spec 33 adt;
            rely is_int64 _t'6;
