...
Definition is_untrusted_intid_body :=
  (Ssequence
    (Sifthenelse (Ebinop Oge (Etempvar _intid tulong)
                   (Econst_long (Int64.repr 32) tulong) tint)
      (Sset _t'1
        (Ecast
          (Ebinop Ole (Etempvar _intid tulong)
            (Econst_long (Int64.repr 511) tulong) tint) tbool))
...
Definition f_is_untrusted_intid := {|
  fn_return := tuint;
  fn_callconv := cc_default;
  fn_params := ((_intid, tulong) :: nil);
  fn_vars := nil;
  fn_temps := ((_t'1, tint) :: nil);
  fn_body := is_untrusted_intid_body
|}.
