...
VOFILES:=$(VFILES:.v=.vo)
VOFILES5=$(patsubst proof/%,%,$(filter proof/%,$(VOFILES)))
...
install:
	cd "proof"; for i in $(VOFILES5); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/RMM/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/RMM/$$i; \
	done
